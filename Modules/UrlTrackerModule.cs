...
namespace InfoCaster.Umbraco.UrlTracker.Modules
{
    ...
    public class UrlTrackerModule : IHttpModule
    {
        ...
        public void Init(HttpApplication context)
        {
            ...
            UmbracoModule.EndRequest += context_EndRequest;
                ...
                if (!_urlTrackerInstalled)
                {
                    lock (_lock)
                    {
                        ...
                        if (!_urlTrackerInstalled)
                        {
                            ...
                            var installer = new UrlTrackerInstallerService() { DontWait = true };
                            try
                            {
        ...
        static void UrlTrackerDo(string callingEventName, bool ignoreHttpStatusCode = false)
        {
            ...
            if (_urlTrackerInstalled && (response.StatusCode == (int)HttpStatusCode.NotFound || ignoreHttpStatusCode))
            {
                ...
                var urlHasQueryString = url.Contains('?'); // invalid querystring (?xxx) without = sign must also be stripped...
                ...
                var shortestUrl = UrlTrackerHelper.ResolveShortestUrl(urlWithoutQueryString);
                ...
                if (domains.Any())
                {
                    ...
                    fullRawUrl = previousFullRawUrlTest = fullRawUrlTest =
                                                            string.Format("{0}{1}{2}{3}", request.Url.Scheme, Uri.SchemeDelimiter, request.Url.Host, request.Url.AbsolutePath);
                ...
                var redirectPassThroughQueryString = true;
                ...
                if (!redirectHttpCode.HasValue)
                {
                    if (!ignoreHttpStatusCode)
                    {
                        // Normal matching (database)
                        // Regex matching
                        var query = "SELECT * FROM icUrlTracker WHERE Is404 = 0 "
                                    + "AND ForceRedirect = @forceRedirect AND (RedirectRootNodeId = @redirectRootNodeId OR RedirectRootNodeId = -1)"
                                    + " AND OldRegex IS NOT NULL ORDER BY Inserted DESC";

                        var data = database.Query<UrlTrackerModel>(
                            query,
                            new object[] { new { forceRedirect = ignoreHttpStatusCode ? 1 : 0 }, new { redirectRootNodeId = rootNodeId } });

                        foreach (var model in data)
                        {
                            var regex = new Regex(model.OldRegex);
                            if (regex.IsMatch(url))
                            {
                                LoggingHelper.LogInformation("UrlTracker HttpModule | Regex match found");
                                if (model.RedirectNodeId.HasValue)
                                {
                                    var redirectNodeId = model.RedirectNodeId.Value;
                                    LoggingHelper.LogInformation("UrlTracker HttpModule | Redirect node id: {0}", redirectNodeId);
                                    var n = new Node(redirectNodeId);
                                ...
                                else if (!string.IsNullOrEmpty(model.RedirectUrl))
                                {
                    ...
                    else
                    {
                        // Forced matching (cache)
                        var forcedRedirects =
                            UrlTrackerRepository.GetForcedRedirects().Where(x => !string.IsNullOrEmpty(x.OldRegex)).ToList();
                        ...
                        foreach (var match in forcedRedirects.Where(x => x.RedirectRootNodeId == -1 || x.RedirectRootNodeId == rootNodeId)
                                                            .Select(x => new { UrlTrackerModel = x, Regex = new Regex(x.OldRegex) }).Where(x => x.Regex.IsMatch(url)))
                        {
                            LoggingHelper.LogInformation("UrlTracker HttpModule | Regex match found");
                            if (match.UrlTrackerModel.RedirectNodeId.HasValue)
                            {
                                ...
                                var n = new Node(match.UrlTrackerModel.RedirectNodeId.Value);
                                if (n != null && n.Name != null && n.Id > 0)
                                {
                        ...
                        var redirectUri = new Uri(
                            redirectUrl.StartsWith(Uri.UriSchemeHttp, StringComparison.OrdinalIgnoreCase)
                                ...
                                : string.Format(
                                    ...
                                    Uri.SchemeDelimiter,
                                    request.Url.Host,
                                    request.Url.Port != 80 && UrlTrackerSettings.AppendPortNumber ? string.Concat(":", request.Url.Port) : string.Empty,
                                    redirectUrl.StartsWith("/") ? redirectUrl.Substring(1) : redirectUrl));
                        if (redirectPassThroughQueryString)
                        {
                            var redirectQueryString = HttpUtility.ParseQueryString(redirectUri.Query);
                            ...
                            var pathAndQuery = Uri.UnescapeDataString(redirectUri.PathAndQuery) + redirectUri.Fragment;
                            redirectUri = new Uri(
                                string.Format(
                                    "{0}{1}{2}{3}/{4}{5}",
                                    redirectUri.Scheme,
                                    Uri.SchemeDelimiter,
                                    redirectUri.Host,
                                    redirectUri.Port != 80 && UrlTrackerSettings.AppendPortNumber ? string.Concat(":", redirectUri.Port) : string.Empty,
                                    pathAndQuery.Contains('?')
                                        ? pathAndQuery.Substring(0, pathAndQuery.IndexOf('?'))
                                        : pathAndQuery.StartsWith("/")
                                            ? pathAndQuery.Substring(1)
                                            : pathAndQuery,
                                    newQueryString.HasKeys() ? string.Concat("?", newQueryString.ToQueryString()) : string.Empty));
                        }

                        if (redirectUri == new Uri(
                                string.Format(
                                    "{0}{1}{2}{3}/{4}",
                                    request.Url.Scheme,
                                    Uri.SchemeDelimiter,
                ...
                {
                    // Log 404
                    if (!UrlTrackerSettings.IsNotFoundTrackingDisabled && !UrlTrackerSettings.NotFoundUrlsToIgnore.Contains(urlWithoutQueryString)
                        && !UmbracoHelper.IsReservedPathOrUrl(urlWithoutQueryString) && request.Headers["X-UrlTracker-Ignore404"] != "1")
                    {
                        var ignoreNotFoundBasedOnRegexPatterns = false;
                        foreach (var regexNotFoundUrlToIgnore in UrlTrackerSettings.RegexNotFoundUrlsToIgnore)
                        {
                        ...
                        if (!ignoreNotFoundBasedOnRegexPatterns)
                        {
                            ...
                            var query = "INSERT INTO icUrlTracker (OldUrl, RedirectRootNodeId, ";
                            if (urlHasQueryString)
                            {
                            ...
                            database.Execute(
                                ...
                                new object[]
                                    {
                                        new { oldUrl = urlWithoutQueryString }, new { redirectRootNodeId = rootNodeId },
                                        new { oldUrlQueryString = request.QueryString.ToString() },
                                        new
                                            {
                                                referrer = request.UrlReferrer != null && !request.UrlReferrer.ToString().Contains(UrlTrackerSettings.ReferrerToIgnore)
        ...
        static void LoadUrlTrackerMatchesFromDatabase(
            HttpRequest request,
            ...
            ref int? redirectHttpCode,
            ref bool redirectPassThroughQueryString)
        {
            var query = "SELECT * FROM icUrlTracker WHERE Is404 = 0 AND ForceRedirect = 0 "
                            + "AND (RedirectRootNodeId = @redirectRootNodeId OR RedirectRootNodeId IS NULL OR RedirectRootNodeId = -1)"
                            + " AND (OldUrl = @url OR OldUrl = @shortestUrl) ORDER BY CASE WHEN RedirectHttpCode = 410 THEN 2 ELSE 1 END, OldUrlQueryString DESC";

            var database = ApplicationContext.Current.DatabaseContext.Database;
            var data = database.Query<UrlTrackerModel>(
                query,
                new object[] { new { redirectRootNodeId = rootNodeId }, new { url = urlWithoutQueryString }, new { shortestUrl = shortestUrl }, });


            foreach (var model in data)
            {
                LoggingHelper.LogInformation("UrlTracker HttpModule | URL match found");
                if (model.RedirectNodeId.HasValue && model.RedirectHttpCode != (int)HttpStatusCode.Gone)
                {
                    var redirectNodeId = model.RedirectNodeId.Value;
                    LoggingHelper.LogInformation("UrlTracker HttpModule | Redirect node id: {0}", redirectNodeId);
                    var n = new Node(redirectNodeId);
                    if (n != null && n.Name != null && n.Id > 0)
                    {
                        var tempUrl = UmbracoHelper.GetUrl(redirectNodeId);
                        redirectUrl = tempUrl.StartsWith(Uri.UriSchemeHttp)
                                        ? tempUrl
                                        : string.Format(
                                            "{0}{1}{2}{3}{4}",
                                            ...
                                            HttpContext.Current.Request.Url.Port != 80 && UrlTrackerSettings.AppendPortNumber
                ...
                redirectPassThroughQueryString = model.RedirectPassThroughQueryString;
                LoggingHelper.LogInformation(
                    "UrlTracker HttpModule | PassThroughQueryString is {0}",
                    redirectPassThroughQueryString ? "enabled" : "disabled");

                NameValueCollection oldUrlQueryString = null;
                if (!string.IsNullOrEmpty(model.OldUrlQueryString))
                {
                    oldUrlQueryString = HttpUtility.ParseQueryString(model.OldUrlQueryString);
                    LoggingHelper.LogInformation("UrlTracker HttpModule | Old URL query string set to: {0}", oldUrlQueryString.ToQueryString());
                }

                if ((urlHasQueryString || oldUrlQueryString != null)
                    && (oldUrlQueryString != null && !request.QueryString.CollectionEquals(oldUrlQueryString)))
                {
        ...
        private static string GetCorrectedUrl(Uri redirectUri, int rootNodeId, string pathAndQuery)
        {
            ...
            if (redirectUri.Host != HttpContext.Current.Request.Url.Host)
            {
                ...
                var siteDomains = domains.Where(x => x.NodeId == rootNodeId).ToList();
                var hosts = siteDomains.Select(x => new Uri(x.UrlWithDomain).Host).ToList();
        ...
        static void LoadUrlTrackerMatchesFromCache(
            HttpRequest request,
            string urlWithoutQueryString,
            bool urlHasQueryString,
            string shortestUrl,
            ...
            ref bool redirectPassThroughQueryString)
        {
            ...
            var redirects = forcedRedirects
                .Where(
                    x => !x.Is404 && (x.RedirectRootNodeId == rootNodeId || x.RedirectRootNodeId == -1)
                        && (string.Equals(x.OldUrl, urlWithoutQueryString, StringComparison.CurrentCultureIgnoreCase)
                            || string.Equals(x.OldUrl, shortestUrl, StringComparison.CurrentCultureIgnoreCase))).OrderBy(x => x.RedirectHttpCode == 410 ? 2 : 1)
                .ThenByDescending(x => x.OldUrlQueryString).ToArray();
            foreach (var forcedRedirect in redirects)
            {
                ...
                if (forcedRedirect.RedirectNodeId.HasValue && forcedRedirect.RedirectHttpCode != (int)HttpStatusCode.Gone)
                {
                    ...
                    var n = new Node(forcedRedirect.RedirectNodeId.Value);
                    if (n.Name != null && n.Id > 0)
                    {
                        var tempUrl = UmbracoHelper.GetUrl(forcedRedirect.RedirectNodeId.Value);
                        redirectUrl = tempUrl.StartsWith(Uri.UriSchemeHttp)
                                        ? tempUrl
                                        : string.Format(
                                            "{0}{1}{2}{3}{4}",
                        ...
                        {
                            var redirectUri = new Uri(redirectUrl);
                            var pathAndQuery = Uri.UnescapeDataString(redirectUri.PathAndQuery);
                            //redirectUrl = pathAndQuery.StartsWith("/") && pathAndQuery != "/" ? pathAndQuery.Substring(1) : pathAndQuery;
