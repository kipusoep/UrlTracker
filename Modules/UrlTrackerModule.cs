    ...
    public class UrlTrackerModule : IHttpModule
    {
        ...
        static void UrlTrackerDo(string callingEventName, bool ignoreHttpStatusCode = false)
        {
            ...
            if (_urlTrackerInstalled && (response.StatusCode == (int)HttpStatusCode.NotFound || ignoreHttpStatusCode))
            {
                ...
                if (UmbracoHelper.IsReservedPathOrUrl(url))
                {
                ...
                {
                    // Regex matching
                    query = "SELECT * FROM icUrlTracker WITH (NOLOCK) WHERE Is404 = 0 AND ForceRedirect = @forceRedirect AND (RedirectRootNodeId = @redirectRootNodeId OR RedirectRootNodeId = -1) AND OldRegex IS NOT NULL ORDER BY Inserted DESC";
        ...
        {
            string query = "SELECT * FROM icUrlTracker WITH (NOLOCK) WHERE Is404 = 0 AND ForceRedirect = 0 AND (RedirectRootNodeId = @redirectRootNodeId OR RedirectRootNodeId IS NULL OR RedirectRootNodeId = -1) AND (OldUrl = @url OR OldUrl = @shortestUrl) ORDER BY CASE WHEN RedirectHttpCode = 410 THEN 2 ELSE 1 END, OldUrlQueryString DESC";
