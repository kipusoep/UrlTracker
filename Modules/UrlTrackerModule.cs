    ...
    public class UrlTrackerModule : IHttpModule
    {
        ...
        static void UrlTrackerDo(string callingEventName, bool ignoreHttpStatusCode = false)
        {
            ...
            if (_urlTrackerInstalled && (response.StatusCode == (int)HttpStatusCode.NotFound || ignoreHttpStatusCode))
            {
                ...
                if (rootNodeId == -1)
                {
                    // no-node redirects are not stored for 'first' root node but with -1 as root node
                    //rootNodeId = -1;
                ...
                if (redirectHttpCode.HasValue)
                {
                    ...
                    if (!string.IsNullOrEmpty(redirectUrl))
                    {
                        ...
                        Uri redirectUri = new Uri(redirectUrl.StartsWith(Uri.UriSchemeHttp) ? redirectUrl : string.Format("{0}{1}{2}{3}/{4}", request.Url.Scheme, Uri.SchemeDelimiter, request.Url.Host, request.Url.Port != 80 ? string.Concat(":", request.Url.Port) : string.Empty, redirectUrl.StartsWith("/") ? redirectUrl.Substring(1) : redirectUrl));
                        ...
                        if (redirectUri == new Uri(string.Format("{0}{1}{2}{3}/{4}", request.Url.Scheme, Uri.SchemeDelimiter, request.Url.Host, request.Url.Port != 80 ? string.Concat(":", request.Url.Port) : string.Empty, request.RawUrl.StartsWith("/") ? request.RawUrl.Substring(1) : request.RawUrl)))
                        {
        ...
        {
            string root = rootNodeId >-1?new Uri(HttpContext.Current.Request.Url, new Node(rootNodeId).Url).AbsolutePath:"/"; // always provide a base url in case url is not an absolute uri
            //urlWithoutQueryString = root.TrimEnd('/') + '/' + urlWithoutQueryString.TrimStart('/');
                ...
                while (reader.Read())
                {
                    ...
                    if (!reader.IsNull("RedirectNodeId"))
                    {
                        ...
                        if (n != null && n.Name != null && n.Id > 0)
                        {
                            ...
                            redirectUrl = tempUrl.StartsWith(Uri.UriSchemeHttp) ? tempUrl : string.Format("{0}{1}{2}{3}{4}", HttpContext.Current.Request.Url.Scheme, Uri.SchemeDelimiter, HttpContext.Current.Request.Url.Host, HttpContext.Current.Request.Url.Port != 80 ? string.Concat(":", HttpContext.Current.Request.Url.Port) : string.Empty, tempUrl);
                    ...
                    if (n != null && n.Name != null && n.Id > 0)
                    {
                        ...
                        if (redirectUrl.StartsWith(Uri.UriSchemeHttp))
                        {
                            ...
                            /*if (redirectUri.Host != HttpContext.Current.Request.Url.Host)
                            {
                                // if site runs on other domain then current, check if the current domain is already a domain for that site (prevent unnessecary redirect to primary domain)
                                List<UrlTrackerDomain> domains = UmbracoHelper.GetDomains();
                                List<UrlTrackerDomain> siteDomains = domains.Where(x => x.NodeId == forcedRedirect.RedirectRootNodeId).ToList();
                                List<string> hosts =
                                    siteDomains
                                    .Select(x => new Uri(x.UrlWithDomain).Host)
                                    .ToList();
